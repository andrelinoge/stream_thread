class RedisServer
  ...
  def subscribe(channel, member)
    ...
    push_message(channel, member, sub_unsub_options(channel)
                 .merge(:message => "#{member.name} is entering into this chat room"))
  end

  def unsubscribe(channel, member)
    @redis.srem(channel.name, member.id)
    push_message(channel, member, sub_unsub_options(channel)
                 .merge(:message => "#{member.name} is exiting from this chat room"))
  end
  ...
  def queue_data(member)
    { :username => member.name, 
      ...
      :bg_color => "bg-color-#{member.name.length % 5}",
      :event    => "push-message" }
  end
